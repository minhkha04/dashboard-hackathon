...
setInterval(() => {
  const fakeCommit = {
    repo_full_name: `SWP391-FALL2025/Repo-team-${Math.ceil(Math.random() * 5)}`,
    commit_sha: Math.random().toString(36).substring(2, 10),
    created_at: getRandomCommitTime(),
  };
...
function getRandomCommitTime() {
  ...
  // Base là hôm nay
  const baseDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 12, 0, 0, 0); // 12:00:00

  // Random số phút trong khoảng 0 → 179 (3 tiếng = 180 phút)
  const randomMinutes = Math.floor(Math.random() * 180); // 0–179 phút
  const commitTime = new Date(baseDate.getTime() + randomMinutes * 60 * 1000);
